...
## Empirical Distribution Function
  ...
  - This distribution $\hat{F}_n$ puts mass of 1/n on each data point
...
## Bootstrap
  ...
  1. Draw a sample $X_1^*, \ldots, X_n^* \sim \hat{F}_n$
    - All this means is to sample $n$ times with replacement from the original data $X_1, \ldots, X_n$.
  2. Compute our statistic of interest $T_n^* = g(X_1^*, \ldots, X_n^*)$
  3. Repeat $B$ times, to get $T_{n,1}^*, \ldots, T_{n,B}^*$
  4. Compute the variance of the statistic $v_{\text{boot}}$, and standard error $\hat{\text{se}}_{\text{boot}} = \sqrt{v_{\text{boot}}}$
...
```{r}
x <- rexp(15)
x.range <- seq(0, 8, length.out = 1000)
x.F <- pexp(x.range)
x.ecdf <- ecdf(x)
